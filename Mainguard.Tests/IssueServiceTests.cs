...
/// <summary>
/// TI-24 (service) — the <see cref="IssueService.IsSupported"/> host/token matrix over a real repo + a
/// temp <see cref="SecureKeyring"/>, plus dispatch: the service parses owner/repo from the origin remote
/// once (through the shared <see cref="HostConnectionResolver"/>, the same path as the PR service) and
/// routes to the GitHub provider over the injected <see cref="HttpClient"/>, carrying the token only in
/// the Authorization header. Unsupported hosts and missing tokens degrade to typed results. No live network.
/// </summary>
public class IssueServiceTests
{
    private const string Token = "ghp_issue_service_matrix_token_xyz";
        ...
        TempRepoFixture fx, string originUrl, string? tokenHost = null)
    {
        ...
        var keyringDir = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "mainguard-issue-keyring-" + Guid.NewGuid().ToString("N"));
    ...
    public void IsSupported_GitHubWithToken_True()
    {
        ...
        Assert.True(new IssueService(git, keyring).IsSupported(repo));
    }
    ...
    public void IsSupported_GitHubWithoutToken_False()
    {
        ...
        Assert.False(new IssueService(git, keyring).IsSupported(repo));
    }
    ...
    public void IsSupported_GitLabWithToken_False_NotImplementedYet()
    {
        ...
        Assert.False(new IssueService(git, keyring).IsSupported(repo)); // stub provider → unsupported
    }
    ...
    public void IsSupported_NoRemote_False()
    {
        using var fx = new TempRepoFixture();
        var git = new GitService();
        var svc = new IssueService(git, new SecureKeyring(System.IO.Path.Combine(System.IO.Path.GetTempPath(), "mainguard-issue-" + Guid.NewGuid().ToString("N"))));
    ...
    public async Task ListAsync_DispatchesToGitHub_WithParsedSlug_AndAuthHeader()
    {
        ...
        var svc = new IssueService(git, keyring, new HttpClient(handler));

        var items = await svc.ListAsync(repo, IssueState.Open, CancellationToken.None);
    ...
    public async Task Operation_WithNoToken_ThrowsAuthenticationRequired()
    {
        using var fx = new TempRepoFixture();
        var (repo, git, keyring) = Arrange(fx, "https://github.com/octocat/hello-world.git", tokenHost: null);
        var svc = new IssueService(git, keyring, new HttpClient(new CapturingHandler()));

        var ex = await Assert.ThrowsAsync<AuthenticationRequiredException>(() =>
            svc.ListAsync(repo, IssueState.Open, CancellationToken.None));

# GitLoom — Git Foundation Audit & Top-Tier Roadmap
...
`GitLoom.Core/Services/GitService.cs` (1048 lines), the graph in
`GitLoom.Core/Graph/CommitGraphRouter.cs`, and the conflict engine (stub) in
`GitLoom.Core/Services/MergeDiffService.cs`.
...
**Where:** `GitLoom.Core/Services/MergeDiffService.cs:18-31`
...
Prefer **throwing a typed exception** over silently committing as "GitLoom
<gitloom@localhost>" (which pollutes history). Replace every
`repo.Config.BuildSignature(...)` and the `??= new Signature("GitLoom",...)` fallbacks with
...
**Where:** `GitLoom.App/ViewModels/RepoDashboardViewModel.cs:175-232` — `Push`, `Pull`,
...
fall over and where GitLoom should win.
...
**Where:** `GitLoom.Core/Services/RepositoryWatcher.cs:53-101`.
...
**Exactly how:** Introduce a small exception hierarchy in `GitLoom.Core/Exceptions/`
(there's already `SshAuthenticationException`):

```
GitLoomException (base)
...
Each item: what it is, why it matters, and exactly how to build it in GitLoom's stack
...
- LibGit2Sharp cannot script an interactive rebase todo list. Drive it through the CLI with
  ...
  todo file GitLoom generated, and `GIT_EDITOR` to a helper that supplies reword messages,
...
**Why:** GitKraken's signature UX. GitLoom already has a fast `CommitGraphCanvas` and router
...
**Why:** This is the "single biggest time saver" per the comparison research, and GitLoom's
...
noted differentiator; GitLoom already has `GitHubAuthClient` + `SecureKeyring` to build on.
...
**Why:** Standard in all four benchmark clients. GitLoom already has `QueryBy(path)`
...
GitLoom's diff is currently plain-text patch content (`GetFileDiff`).

...
/// <summary>
/// Host-agnostic CI / checks-status service (T-26). Sibling of <see cref="PullRequestService"/> and
/// <see cref="IssueService"/>: it resolves the repo's origin host + stored token and parses
/// <c>owner/repo</c> through the <b>same shared</b> <see cref="HostConnectionResolver"/> (no duplicate
/// host/token resolver), then dispatches to the matching internal <see cref="ICheckProvider"/> (GitHub v1;
/// GitLab/Bitbucket/Azure DevOps stubs).
///
...
/// </summary>
public sealed class CheckStatusService : ICheckStatusService
{
    ...
    public CheckStatusService(IGitService git, ISecureKeyring? keyring = null, HttpClient? httpClient = null)
    {
    ...
    public Task<CommitChecks> GetChecksAsync(string repoPath, string sha, CancellationToken ct)
    {
        ...
        return provider.GetChecksAsync(slug, token, sha, ct);
    }

    public Task RerequestAsync(string repoPath, long checkRunId, CancellationToken ct)
    {
        var (provider, slug, token) = Resolve(repoPath);
        return provider.RerequestAsync(slug, token, checkRunId, ct);
    }
    ...
    // HostConnectionResolver (same path as PR/issue services); only the provider dispatch is checks-specific.
    private (ICheckProvider Provider, RepoSlug Slug, string Token) Resolve(string repoPath)
    {
        ...
        var provider = ResolveProvider(kind);
        if (provider is not { IsImplemented: true })
            throw new GitOperationException($"CI checks are not available for '{host}'.");
    ...
    private ICheckProvider? ResolveProvider(HostKind kind) => kind switch
    {
        HostKind.GitHub => new GitHubCheckProvider(_http),
        HostKind.GitLab => new GitLabCheckProvider(),
        HostKind.Bitbucket => new BitbucketCheckProvider(),
        HostKind.AzureDevOps => new AzureDevOpsCheckProvider(),
        _ => null,
    };
}

#!/bin/sh
# gitloom-egress-proxy entrypoint (P2-07). Waits for the daemon to render the allowlist config, then
...
i=0
while [ ! -f /etc/gitloom/tinyproxy-filter ] && [ "$i" -lt 60 ]; do
    sleep 1
    i=$((i + 1))
done

/etc/gitloom/reload.sh || true

...
**Sources of truth.** DESIGN.md and PRODUCT.md govern the system and register; the Voice & Delight Bible (`docs/creative/Mainguard_Voice_And_Delight_Bible.md`) governs every string quoted here (cited by rule id, e.g. **V-4**, **C-1**); AGENTS.md pins every claim to a real view, ViewModel, service, or token. Nothing here invents a color, radius, spacing value, or motion outside the system. Where this document proposes copy, it follows the Bible's five-question gate (Appendix A there).

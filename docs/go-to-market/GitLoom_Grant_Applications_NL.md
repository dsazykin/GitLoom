# GitLoom — Grant & Subsidy Application Drafts (Netherlands / Twente stack)
...
**Reads from:** `GitLoom_Master_Market_Document_2026-07.md` Part X (the funding stack, walkthroughs, and sources), Part IX (Novel-T/ecosystem), Part XI (BV prerequisite), Part XII (hiring/WBSO interaction), and `docs/phase-2/` for the technical substance every application cites.
...
**Fit:** GitLoom's daemon/sandbox/merge-queue development is textbook S&O — technically new software development with demonstrable technical bottlenecks (Part X's own examples are drawn from this project).
...
> **Company.** GitLoom B.V. (Enschede) — a native Git client for Windows (shipped; 1,042 automated tests) evolving into a verification and merge-governance layer for AI-coding-agent output (in development).
>

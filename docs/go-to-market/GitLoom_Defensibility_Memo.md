# GitLoom — Defensibility Memo (why this doesn't get taken from us)
...
**Companion:** `GitLoom_Objection_Handling.md` — the same material, cut per objection for live use.
...
- **The name.** MergeLoom's collision makes "GitLoom" a risk item, not an asset; every positioning line survives a rename by design (Narrative §2.2).

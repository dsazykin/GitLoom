...
> **Verification profile:** Fully automated CI guardrails — the WAN-latency job runs the unchanged P2-14 suite per release; no human step.
> The deliverable is CI configuration + proto-descriptor sweeps; 'tests about tests' per TI-P2-25.
>
> **Source of truth:** §P2-25 of `docs/phase-2/implementation_plans/GitLoom_Master_Implementation_Document_v2.md`. **This task
...
|---|---|
| [Master doc](../phase-2/implementation_plans/GitLoom_Master_Implementation_Document_v2.md) §P2-25 | Contract, invariants, edge rows, rejection triggers -- the source of truth (note: the doc moved on 2026-07-11; older copies of this plan cited `docs/GitLoom_Master_Implementation_Document_v2.md`) |
| [Test strategy v2](../phase-2/implementation_plans/GitLoom_Test_Implementation_Strategy_v2.md) **TI-P2-25** | The binding expansion of this plan's test contract -- "a feature PR that does not satisfy its TI section is incomplete by definition." Where the table below and TI-P2-25 differ, implement the union. The §A.4 shared fixtures (`DaemonFixture`, `ScriptedAgentHarness`, `FakeModelEndpoint`, `DualRepoFixture`, `SandboxFixture`, `AuditProbe`) are infrastructure contracts: hand-rolling what a fixture provides is a review rejection |
...
| **Create** | `docs/cloud-guardrails.md` (what the guardrails enforce; the P3-06 pointers: mTLS + user auth replacing the session token, per-tenant encryption at rest, `RemoteEnvironment` picker (local VM \| cloud), repo sync via `git push gitloom-cloud` over HTTPS, where `gitloom-cloud` is the cloud substrate's resolved `SyncRemote.Name` — the same quarantine-sync-remote role as WSL2's `gitloom-vm`, per ESC B1 decision SC-2 / P2-06 — recorded as future contract, not built) |

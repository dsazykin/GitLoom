# Mainguard — Test Implementation Strategy v2 (Phase 2 / Agent Platform)

**Date:** 2026-07-07
**Companion to:** `Mainguard_Master_Implementation_Document_v2.md` — every task `P2-xx` / `P2-Cx` / `P3-xx` there has a section `TI-P2-xx` / `TI-P2-Cx` / `TI-P3-xx` here, grouped by the master doc's categories (§3 build order). A feature PR that does not satisfy its TI section is incomplete by definition.
...
**Relationship to the inline "Required tests" blocks:** Master Doc v2 embeds a summary **Required tests** block in each task. This document is the *expansion* of those blocks into concrete, named test cases — the same relationship v1's `Mainguard_Test_Implementation_Strategy.md` had to Master Doc v1. The inline block is the contract's headline; this document is its case list. If the two ever disagree, the master doc's block wins — and fix the drift here in the same PR.
**v1 strategy still applies:** `Mainguard_Test_Implementation_Strategy.md` §A (conventions, tiers, assertion rules, headless render harness) remains binding for everything it covers. This document only *adds* the infrastructure and specifications Phase 2 needs; it repeats v1 rules only where Phase 2 changes or extends them.
...
| **Daemon in-proc** (new) | `Mainguard.Server` hosted via `WebApplicationFactory`, real gRPC channel in-process, `--local-dev` semantics | Every RPC surface: auth, streaming, deadlines, error mapping, interceptor behavior (G-13, G-18, P2-23 enforcement) |
...
New test project **`Mainguard.Server.Tests`** (added to `Mainguard.slnx`, referenced packages: `Microsoft.AspNetCore.Mvc.Testing`, `Grpc.Net.Client`) for the daemon in-proc tier. Pure daemon *logic* (merge-queue state machine, gateway bucket math, hash chain, classifiers) stays in `Mainguard.Agents` and is tested from **`Mainguard.Tests`** as ordinary pure units — the split mirrors P2-02's rejection trigger (no business logic in gRPC classes). Record the new project in `AGENTS.md`'s Repository Map in the same PR.

Shared fixtures (all under `Mainguard.Server.Tests/Fixtures/` unless noted):

1. **`DaemonFixture`** — in-proc `Mainguard.Server` host + authenticated `GrpcChannel`; exposes the session token, a wrong-token channel factory, and a log capture sink for the G-13 field-mask assertions. Every daemon in-proc test uses it; hand-rolled hosts are a bug.
2. **`ScriptedAgentHarness`** — a tiny cross-platform console binary (checked into `Mainguard.Tests/TestTools/ScriptedAgent/`) that speaks the P2-09 control protocol (`[IPC_UPDATE_REQUESTED]` → `[IPC_UPDATE_READY]`), executes a scripted timeline (write file → commit → emit output → exit / hang / crash), and can be configured to ignore yield requests (timeout paths). This is the single most important Phase-2 harness: lifecycle, queue, coordinator, curation, checkpoints, repair-loop, and Vibe tests all drive it instead of a real CLI.
...
4. **`DualRepoFixture`** — a Windows-side `TempRepoFixture` plus an ext4-style bare mirror (plain second directory on CI), wired the way P2-06 provisions: bare clone, `agent/<id>` worktrees, `mainguard-vm` remote. Provides `CaptureRefState()` (v1 TI-19 helper, promoted here) for byte-identical round-trip assertions.
...
- **PR gate (Windows leg, exists today):** build + full `Mainguard.Tests` + format — the v1 < 3-minute budget still applies; Phase-2 pure/VM/repo tests land here.
- **PR gate (Linux leg, new with P2-02):** build `Mainguard.Server` linux-x64 + `Mainguard.Server.Tests` + `RequiresDocker` suites (the runner has Docker). The P2-06/P2-07 container tests are *PR-blocking* on this leg — sandbox invariants are launch-tier security, not nightly hygiene.
...
12. Guard: `grep -rn "llm_" Mainguard.App.Shell/ | grep -i "preferences\|settings.json"` → 0 hits (reviewer script; not automated as a source-grep test — v1 TI-16 rule).

---

## TI-P2-02 — Daemon + gRPC v1 contract

**Files:** `Mainguard.Server.Tests/DaemonAuthTests.cs`, `TerminalStreamRpcTests.cs`, `DaemonClientReconnectTests.cs`, `LoggingMaskTests.cs` (daemon in-proc via `DaemonFixture`).
...
## TI-P2-05 — MainguardOS bootstrapper
...
3. `Provision_PathWithSpacesAndUnicode_ShouldHashAndRegisterCorrectly` — including the `mainguard-vm` remote URL registration idempotence (run twice → one remote).
...
7. `CommitRoundTrip_ShouldBeByteIdentical_WindowsToVmAndBack` — commit in the agent worktree → `git fetch mainguard-vm && git merge agent/<id>` on the Windows fixture → tree SHA and blob bytes equal (`CaptureRefState` before/after).
...
5. `TarballBuild_ShouldBeHashStable_GivenPinnedInputs` — CI job builds `MainguardOS.tar.gz` twice → identical hash; `/etc/mainguardos-release` version stamped.
...
3. `DeepLink_MainguardScheme_ShouldCarryNoSecrets` — code-path test: the deep-link builder API accepts no token-typed inputs; plus the reviewer grep for tokens in `mainguard://` construction sites.
...
8. Manual matrix (PR evidence): uninstall on a machine with a personal distro — `--terminate` → poll → `--unregister` only ours; registry/tasks/appdata removed; user repo + optional `mainguard-vm` remote handling; G-12 grep.
...
4. `Lessons_ShouldLiveInRepoFile_HumanEditableAndPrAble` — `.mainguard/lessons.md` round-trip; repo without `.mainguard/` → created on first enable.
...
6. `Snapshots_ShouldBePinnedUnderRefsMainguardSnapshots_AndSurviveGitGc` — run `git gc --prune=now` → snapshot SHAs still resolvable; `RetentionPolicy_ShouldPruneOldSnapshots`.
...
1. `AgentCommit_ShouldBeSignedWithMintedAgentKey_VerifiableViaT15` — verify-commit fixture; identity `agent-<adapter>@mainguard-daemon`.
...
4. **Patches/WIP share:** `FormatPatch_Am_ShouldRoundTripByteIdentically`; `SharePatchRef_ShouldPushRefsMainguardPatches_AndImportFlowRestoresIt` (bare-remote fixture).

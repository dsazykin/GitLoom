# Mainguard — WSL2 Substrate (B2 — the `SubstrateId = "wsl2"` implementation)

**Status:** Draft for review · **Revision:** 2026-07-11 (initial cut) · **Subordinate to:** `docs/phase-2/implementation_plans/Mainguard_Master_Implementation_Document_v2.md` (the binding spec) · **Implements:** `docs/phase-2/Mainguard_Environment_Substrate_Contract.md` (ESC / B1 — the umbrella) · **Consistent with:** `docs/phase-2/Mainguard_Orchestration_Protocol_Spec.md` (OPS v1). Where this document and the master doc disagree, **the master doc wins** and the disagreement is drift to be fixed here — the same precedence rule ESC applies to itself.

This is the WSL2 realization of the platform-agnostic `IAgentEnvironment` umbrella. It is one of the sibling B-docs ESC §0.3 enumerates; it renames nothing (`MainguardOsBootstrapper`, `IRepoProvisioner`, `IAgentWorktreeManager`, `SandboxEngine`, `EgressProxyConfigurator`, `mainguard-vm`, `MainguardEnv`, `ProvisionResult` keep their names and semantics from the master doc) and introduces no parallel system. The only new type is `Wsl2AgentEnvironment` — the concrete `IAgentEnvironment` for `SubstrateId = "wsl2"`, landed additively inside P2-06 per ESC SC-3, delegating health/upgrade into P2-05 bootstrap logic and teardown into P2-22.
...
This document is the `SubstrateId = "wsl2"` implementation of the ESC contract. It **realizes ESC-I1…ESC-I9** on a WSL2 VM (`MainguardEnv`) and **runs the ESC §4 `SubstrateConformance` suite** against `Wsl2AgentEnvironment`, pasting the WSL2 results (§5) and filling the ESC §5 metric framework with WSL2 numbers (§3). It owns exactly the WSL specifics ESC §0.3 delegates to B2: 9P object transfer, the `\\wsl.localhost\MainguardEnv\...` UNC remote, the `.wslconfig` merge, and the `wsl --terminate MainguardEnv` lifecycle dialect.
...
|---|---|---|---|
| ESC-I1 Git-objects-only boundary | the sandbox mounts only the **ext4** worktree under `~/mainguard/worktrees/...`; the host repo reaches the ext4 bare mirror through **9P object transfer only** (`git clone --bare /mnt/c/...` then `git fetch`), never a bind mount (G-11) | [STRUCT] | 1, 2 |
| ESC-I2 control-plane-only | UI speaks `mainguardd` gRPC over loopback; `Wsl2AgentEnvironment` and its members live daemon-side and are never referenced from `Mainguard.App.Shell` (G-18) | [STRUCT] | 6 |
| ESC-I3 quarantine remote | each agent worktree's `origin` is the daemon-owned `~/mainguard/repos/<hash>.git` bare and **only** that; the VM holds no credential for, and no remote pointing at, the user's real host (P2-06 quarantine ext.) | [STRUCT] | 3 |
| ESC-I4 no host-destroying teardown | `TeardownScope.Instance` = `wsl --terminate MainguardEnv` → poll → `wsl --unregister MainguardEnv`; **never `wsl --shutdown`** (G-12) — personal distros untouched | [STRUCT]+[CHECK] | 4, WT-16 |
| ESC-I5 no runtime image build | `SandboxEngine` starts a prebuilt, versioned base image (`/etc/mainguardos-release`); toolchains sideload via `devbox add` (G-16) — no `docker build` path | [STRUCT] | 7 |
...
| ESC-I8 transport-agnostic handles | `SyncRemote.Url` is the opaque `\\wsl.localhost\MainguardEnv\...` UNC handle the Windows side registers verbatim; timeouts `RttBudget`-scaled, survive the P2-25 WAN job (G-14) | [CHECK] | 1, 10 |
...
|---|---|---|
| **P2-05** | `MainguardOsBootstrapper`: `.wslconfig` merge, `dockerd` boot, `mainguardd` health-check, idempotent resume — the `HealthCheckAsync`/`UpgradeAsync` backing (§1.2) | G-12; ESC-I4, ESC-I9 |
...
| **P2-21** | Installer part 1: `SystemDiagnostics` preflight, OOBE/OS enablement, `MainguardOS.tar.gz` payload, in-place upgrade (§4) | ESC-I9 |
...
The WSL2 shape is ESC §2's reference topology with every abstract node bound to its WSL2 realization. `MainguardEnv` is the SUBSTRATE boundary; the host↔bare edge is 9P object transfer; the sandbox mount source is the ext4 worktree; the sync remote is a `\\wsl.localhost` UNC handle.
...
flowchart LR
    ...
    subgraph ENV["MainguardEnv — WSL2 VM (Wsl2AgentEnvironment, SubstrateId=wsl2)"]
        D["mainguardd (TCB)<br/>MergeQueue · Gateway · Audit"]
        BARE[("ext4 quarantine bare<br/>~/mainguard/repos/&lt;hash&gt;.git")]
        subgraph SBX["agent sandbox — Docker-in-WSL2 (hardened, G-15)"]
            SUP["AgentSupervisor"]
            CLI["agent CLI + PTY"]
            WT[("ext4 worktree<br/>~/mainguard/worktrees/&lt;repo&gt;/&lt;agentId&gt;<br/>origin = quarantine bare")]
        end
    ...
    HREPO -- "git fetch mainguard-vm<br/>9P objects only — no watching (G-11/ESC-I1)" --> BARE
...
`Wsl2AgentEnvironment` drives `MainguardEnv` through a check-then-act state machine (P2-05 idempotence). Every step is resumable: a partial bootstrap re-enters at the first failed check. The `Instance` teardown path uses `--terminate` → poll → `--unregister` and **never** `wsl --shutdown` (G-12 / ESC-I4).

```mermaid
stateDiagram-v2
    [*] --> Absent
    Absent --> Importing: wsl --list --quiet shows no MainguardEnv
    Importing --> MergingConfig: import MainguardOS.tar.gz (versioned tarball)
    Absent --> MergingConfig: MainguardEnv already present
    ...
    DockerdStarting --> MainguarddStarting: dockerd socket ready (waited, not assumed — WT-5)
    MainguarddStarting --> Running: HealthCheckAsync green (gRPC + docker info + disk headroom)
    Running --> Running: HealthCheckAsync (idempotent, read-only)
    Running --> Terminating: TeardownScope.Instance
    Terminating --> Stopped: wsl --terminate MainguardEnv, poll until stopped
    Stopped --> Unregistered: wsl --unregister MainguardEnv (uninstall only, P2-22)
    Unregistered --> [*]
...
| 3 | **WSL v1, not v2** (default version 1 or distro on v1) | diagnostics parse `wsl -l -v` version column; `MainguardEnv` imported as v2 explicitly (`--version 2`) | if a stale v1 `MainguardEnv` exists, `--set-version MainguardEnv 2`; set default `--set-default-version 2` before import | `WT-2` (parser PR; convert MM) |
...
| 12 | **Windows fast-startup / sleep / hibernate kills the VM mid-session** (hybrid shutdown or hibernate stops `MainguardEnv`; containers vanish) | on reconnect the daemon gRPC is unreachable → client re-runs `MainguardOsBootstrapper` (idempotent); `SwarmReconciler` (P2-08) reconciles Docker on boot | re-boot `MainguardEnv` (idempotent, §1.2); reconciler marks vanished agents `Dead`, prunes their worktrees, surfaces disposal in the UI; ext4 repos+worktrees on disk survive the VM stop; see OPEN DECISION WSL-3 (auto-resume policy) | `WT-10` (MM) |
...
| 15 | **`wsl --terminate` / `kill -9` recovery + session-leader reattach** (VM or `mainguardd` killed out from under a live session) | client detects gRPC disconnect; on reconnect it re-boots (idempotent) and re-attaches to the running `mainguardd` session; `SwarmReconciler` adopts live containers | idempotent re-boot brings `MainguardEnv` + `dockerd` + `mainguardd` back; reconciler adopts orphan-but-live containers and prunes dead ones (P2-08); the client re-subscribes `StreamAgentEvents` from its cursor (OPS A4) — a killed VM never wedges the next launch (the P2-05 re-run is a no-op past the green checks) | `WT-13` (reconciler PR leg + reattach MM) |
| 16 | **versioned-tarball VM upgrade must preserve repos + worktrees** (`vN → vN+1` in-place image upgrade) | `UpgradeAsync(TargetReleaseId, PreserveProvisionedRepos: true)`; `/etc/mainguardos-release` compared; a no-op when already current (`Applied=false`) | in-place upgrade re-imports the new `MainguardOS.tar.gz` while preserving the `~/mainguard` repo+worktree tree (P2-21); the automated test asserts repos/worktrees survive the bump; idempotent re-run | `WT-14` (upgrade harness PR-where-CI-allows + MM), ESC test 5 |
| 17 | **`wsl --shutdown` would kill the user's personal distros (G-12)** | grep guard over `Mainguard.Agents`/`Mainguard.Server`/`installer` for `wsl --shutdown`/`--shutdown` → 0 hits; `TeardownScope` has no host member | structurally absent: the only instance-teardown verb is `wsl --terminate MainguardEnv` → poll → `--unregister` (P2-22); there is no code path to a host-wide stop (ESC-I4 [STRUCT]) | `WT-16` (grep guard, **PR**), ESC test 4 |
...
3. **RttBudget stated.** WSL2 numbers are measured under the **loopback** budget (`MainguardEnv` on the same host, sub-ms to low-ms RTT). The P2-25 WAN job re-runs the same flows under `tc netem` 80 ms to prove no spurious timeout (ESC-I8); those numbers are reported separately and are not the WSL2 steady-state figures.
...
> | Substrate cold start | import tarball + boot + `dockerd` + `mainguardd` health from nothing: **target < 60 s** (P2-05 fresh-import target) | `MainguardEnv` already up: `HealthCheckAsync` green in ≈ 1 s |
...
| `Repo` | remove `~/mainguard/repos/<hash>.git` + its worktrees | one provisioned repo only; other repos untouched |
| `Instance` | `wsl --terminate MainguardEnv` → poll until stopped → `wsl --unregister MainguardEnv` | `MainguardEnv` only; **never `wsl --shutdown`**; personal distros untouched (uninstall test) |

Uninstall additionally removes Explorer context menus, registry keys, Scheduled Tasks, and appdata; the user's repo is untouched; the `mainguard-vm` remote removal is offered, not forced (P2-22).

## 4.4 Upgrade (`UpgradeAsync`)

`UpgradeAsync(SubstrateUpgradeRequest(TargetReleaseId, PreserveProvisionedRepos), ct)` compares `/etc/mainguardos-release`; if current it returns `UpgradeResult(Applied:false, ...)` (idempotent no-op, ESC test 5). Otherwise it performs the in-place `vN → vN+1` tarball upgrade preserving the `~/mainguard` repo+worktree tree (§2 WT-14). CVE patch cadence documented in `docs/mainguardos-updates.md`.

## 4.5 ARM64 policy

`SystemDiagnostics` treats **ARM64 as an explicit unsupported gate** for v1 (P2-21): the preflight fails fast with a clear "unsupported architecture" message and no partial mutation. No ARM64 `MainguardOS.tar.gz` is built for v1; the gate is a documented, honest hard-stop, not a silent degrade. Revisiting ARM64 is future work outside B2.

---

# 5. WSL conformance results

Two suites run against `Wsl2AgentEnvironment`: the ESC §4 `SubstrateConformance` suite (tests 1–11, generalized off the P2-06/P2-07 required tests) and the WSL-specific `WT-*` tests (§2). Each has a single pass/fail assertion, a trait, and a leg. Per the test strategy (A.3): a `RequiresWsl` test never gates a PR — it runs in the manual matrix / dedicated Windows runner — and **no test requires both WSL and Docker-in-WSL in the PR gate**. The ESC §4 Docker legs therefore run on the **Linux CI leg** (Docker present, PR-blocking) to prove the invariant logic; their **wsl2-native confirmation** (the identical assertions with Docker inside `MainguardEnv`) is `RequiresWsl` + `RequiresDocker` and rides the manual matrix.

## 5.1 ESC §4 `SubstrateConformance` suite — WSL2 leg

| # | Test id | Trait | WSL2 pass assertion | Automatable vs MM |
|---|---|---|---|---|
| 1 | `GitObjectsRoundTrip_ShouldBeByteIdentical` | `RequiresDocker`+`SubstrateConformance` | agent commits in the ext4 worktree; Windows `git fetch mainguard-vm && git merge agent/<id>` yields **byte-identical** tree/blob SHAs (P2-06 inv.2) | Linux CI leg **PR**; wsl2-native MM |
...
| 6 | `ControlPlaneOnly_UiHasNoSubstrateHandle` | (pure) `SubstrateConformance` | no `Docker.DotNet`/`Porta.Pty`/`IAgentEnvironment` reference in `Mainguard.App.Shell`; every substrate act is an RPC; `NoRpcWritesWindowsRepo` holds | pure **PR** |
...
| `WT-2` `WslV1NotV2_ShouldDetectAndConvert` | `RequiresWsl` (+ parser pure) | `wsl -l -v` fixtures classify a v1 distro; import forces `--version 2`; stale v1 `MainguardEnv` is converted | parser **PR**; convert MM |
...
| `WT-4` `WslConfigMerge_ShouldPreserveUserKeys` | (pure) | `WslConfigMerger` preserves every pre-existing key/section/comment; adds only `[wsl2]` Mainguard keys; backup written | pure **PR** |
...
| `WT-14` `VersionedTarballUpgrade_ShouldPreserveReposAndWorktrees` | `RequiresWsl` (+ harness where CI allows) | `vN → vN+1` in-place upgrade preserves `~/mainguard` repos+worktrees; a same-release upgrade is a no-op | harness **PR-where-CI-allows**; MM |
...
| `WT-16` `NeverWslShutdown_GrepGuard` | (pure) | grep over `Mainguard.Agents`/`Mainguard.Server`/`installer` for `wsl --shutdown`/`--shutdown` → **0 hits**; `TeardownScope` has no host member (ESC-I4/G-12) | pure **PR** |
...
- **Manual matrix / dedicated Windows runner (`RequiresWsl`, never a PR gate):** the wsl2-native confirmation legs of ESC tests 1–5, 8, 9, and `WT-7/8/9/10/11/13/14` — the failure modes only a real `MainguardEnv` with Docker-in-WSL2 can exercise.
...
> **OPEN DECISION [WSL-3]:** when Windows fast-startup / hibernate stops `MainguardEnv` mid-session (§2 WT-10, WT-13), the ext4 repos+worktrees survive but every agent container is gone. Should the daemon **auto-resume** the interrupted agents on re-boot, or mark them `Dead` and let the human re-spawn?
...
> **OPEN DECISION [WSL-4]:** the sync-remote URL uses the `\\wsl.localhost\MainguardEnv\...` UNC alias, but older Windows builds expose the VM only under the legacy `\\wsl$\MainguardEnv\...` alias, and the alias's provider is not ready the instant the VM boots (§2 WT-6).
> **Recommendation:** `ResolveSyncRemote` emits **`\\wsl.localhost\...`** as the canonical form (the current, documented alias) and the provisioner probes the resolved UNC path with a **bounded readiness wait** (modeled on #156) before registering `mainguard-vm`; if `\\wsl.localhost` is unavailable on an older build, fall back to the `\\wsl$` alias for the same path. The URL stays an opaque handle (ESC-I8): the Windows side registers whatever `SyncRemote.Url` resolves to, never a hardcoded string.
...
*End of Mainguard WSL2 Substrate (B2, draft 2026-07-11).*

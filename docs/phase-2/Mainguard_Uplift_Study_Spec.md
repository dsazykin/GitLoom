# Mainguard — The Uplift Study & Quality Bench (Phase 5 spec)
...
> *through Mainguard's pipeline* is safer and more correct than the same model's ungoverned output —
...
Mainguard does not change the model. The same agent CLI writes the same first draft with or without
the product. What Mainguard adds is the **pipeline wrapped around the agent** — verification (P2-10),
...
> *actually merges through Mainguard* has a lower escaped-defect rate and equal-or-higher useful yield
> than the same model's ungoverned output.
>
> **H0 (null, stated honestly):** Mainguard's pipeline produces no statistically significant change in
> escaped-defect rate or useful yield versus the ungoverned baseline.

Two design rules follow and govern everything below:
1. **The model is held constant across every arm.** The only variable is how much of Mainguard wraps
...
The rigorous heart of the study: **treat Mainguard's pipeline as a binary classifier** that, for each
...
to Mainguard's P2-10 verification) and a `held-out` set (sealed, used only by the grader `IOracle`). If
...
(`Assembly.LoadFrom` of a co-located `Mainguard.Bench.Oracle.dll`) needs no compile-time reference. The
...
```mermaid
flowchart LR
  subgraph arms["Mainguard.Bench.Arms  (references Mainguard.Agents pipeline + Contracts)"]
    A[IArmDriver A0..A5]
  end
  subgraph oracle["Mainguard.Bench.Oracle  (separate assembly + process)"]
    O[IOracle / SealedOracleStore]
  end
  subgraph shared["Mainguard.Bench.Contracts"]
    C[RunConfig · RunResult · BenchTask · SealedRef]
  end
  R[Mainguard.Bench runner/CLI]
...
1. **[CHECK] No reference edge (necessary, not sufficient).** `Mainguard.Bench.Arms` has **no** assembly
   or project reference to `Mainguard.Bench.Oracle`, so arm *source* cannot mention `IOracle`,
...
set of `Mainguard.Bench.*` assemblies (not shipped in the app; linux-x64, runs in CI and the daemon
...
|---|---|---|
| `Mainguard.Bench.Contracts` | (nothing but BCL) | `RunConfig`, `RunResult`, `BenchTask`, `SealedRef`, enums — data only |
| `Mainguard.Bench.Arms` | `Mainguard.Agents` (real pipeline), `Contracts` | `IArmDriver` A0..A5 composing P2-06/P2-10/P2-11/P2-35 types |
| `Mainguard.Bench.Oracle` | `Contracts` **only** | `IOracle`, `SealedOracleStore`, grader; **runs as its own process under a distinct uid, sealed corpus mounted only in its namespace, `OracleKey` resolution gated on an oracle-only secret** |
| `Mainguard.Bench.Metrics` | `Contracts` | pure `BenchMetrics` + `BenchStats` (no IO — like `Analytics`) |
| `Mainguard.Bench` | `Arms`, `Metrics`, `Contracts`, spawns `Oracle` process | runner/CLI, corpus manager, report generator |
...
```csharp
// Mainguard.Bench.Contracts/RunConfig.cs
...
// Mainguard.Bench.Arms/IArmDriver.cs
...
// Mainguard.Bench/IBenchRunner.cs
...
```csharp
// Mainguard.Bench.Contracts/OracleVerdict.cs
...
// Mainguard.Bench.Oracle/IOracle.cs   (this assembly is NOT referenced by Mainguard.Bench.Arms — [STRUCT])
...
// Mainguard.Bench.Oracle/SealedOracleStore.cs
...
```csharp
// Mainguard.Bench.Contracts/BenchTask.cs
public sealed record BenchTask(
    ...
    SealedRef HeldOutTests,                       // opaque — resolvable ONLY inside Mainguard.Bench.Oracle
...
// Mainguard.Bench/ICorpusIngestor.cs
...
// Mainguard.Bench/IContaminationScanner.cs
...
// Mainguard.Bench.Arms/IHazardInjector.cs
...
Pure, no IO, in `Mainguard.Bench.Metrics` — pinned against reference outputs exactly like
`ChangelogGenerator`/`ChurnStats` (Analytics). Every number is reproducible from the pinned
`RunResult` set.

```csharp
// Mainguard.Bench.Metrics/BenchMetrics.cs  (pure)
...
// Mainguard.Bench.Metrics/BenchStats.cs  (pure)
...
```csharp
// Mainguard.Bench/PreRegistration.cs
...
// Mainguard.Bench/EvidenceBundle.cs
...
`Mainguard.Agents/Agents/**`, `Mainguard.Agents/Review/**`, `Mainguard.Agents/Audit/**`, `Mainguard.Agents/Safety/**`)
plus a nightly wider slice. It fails the build if escaped-defect rate regresses beyond tolerance.

```csharp
// Mainguard.Bench.Metrics/QualityBenchGate.cs  (pure)
...
2. **[STRUCT] The held-out oracle is never visible to the pipeline.** `Mainguard.Bench.Arms` has no
   reference to `Mainguard.Bench.Oracle`; `SealedRef` carries no content; grading happens out-of-process
...
5. **[STRUCT] Arms route through the real shipped pipeline services**, never a bench reimplementation
   (`Arms` references `Mainguard.Agents`; there is no bench verifier/classifier type).
...
- **A bench that reimplements pipeline logic instead of driving the shipped services** — any type in
  `Mainguard.Bench.*` that classifies risk, evaluates a diff, or runs verification itself, rather than
  calling P2-10/P2-11/P2-35.
- Dropping failed/negative runs from the denominator (except `Oracle.Valid=false`, which is *invalid*,
  not negative — see §7).
- A `Mainguard.Bench.Arms → Mainguard.Bench.Oracle` reference edge appearing in any PR (breaks §5.2).
...
2. **The escaped-defect reduction:** "code merged through Mainguard had X% lower escaped-defect rate than
   ungoverned output (held-out oracle, p<…, 95% CI …), across N tasks and M models."
3. **The hazard floor:** "Mainguard blocked K/K planted hazards that reach main ungoverned (0% caught)."
...
`Mainguard.Bench.*` assembly graph as the load-bearing [STRUCT] oracle-isolation boundary (§3.3, §4, §5.2)

# Mainguard Performance Hotspot Register

**Status:** binding for Phase 2 · **Scope:** the paths whose *scale* — not correctness — is the risk · **Subordinate to** `Mainguard_Master_Implementation_Document_v2.md`; consistent with `Mainguard_Orchestration_Protocol_Spec.md` (OPS) §2.8/§2.9 and `Mainguard_Environment_Substrate_Contract.md` (ESC) §5.
...
Budgets are meaningless without a machine. Two profiles, both already in the test topology (`Mainguard_Test_Implementation_Strategy_v2.md` §CI legs):
...
```mermaid
flowchart LR
    subgraph WIN["Windows client (Mainguard.App.Shell)"]
    ...
    subgraph VM["Daemon (Mainguard.Server, WSL VM / cloud pod)"]
...
1. **Pure micro-benches (new project: `Mainguard.Benchmarks`, BenchmarkDotNet).** H2 router, H9 bucket math, H12 hash-chain, H13 matcher. These are deterministic, allocation-and-time bounded, and belong on the PR leg. *No such project exists today* — it is the one new piece of infrastructure this register requires (see [PERF-2]).
...
> **Recommendation:** add a **`Mainguard.Benchmarks` project (BenchmarkDotNet), not in `Mainguard.slnx`** (like the existing scratch projects), invoked by a dedicated CI step with a **regression gate on a checked-in baseline JSON** (allowlist-shrinks discipline, mirroring the P2-04 known-failures pattern). Micro-benches must not run inside `dotnet test` (they would blow the < 3-min PR budget as timing-dependent xUnit facts — a v1 rejection trigger).
...
- **Infrastructure:** the `Mainguard.Benchmarks` project + baseline-diff CI step ([PERF-2]); calibration-baseline harness for ratio-based budgets ([PERF-3]).

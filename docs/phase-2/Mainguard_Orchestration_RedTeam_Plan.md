# Mainguard — Orchestration Red-Team Plan (RT v1)

**Status:** Draft for review · **Revision:** 2026-07-11 (initial cut) · **Subordinate to:** `docs/phase-2/implementation_plans/Mainguard_Master_Implementation_Document_v2.md` (the binding spec) · **Attacks:** `docs/phase-2/Mainguard_Orchestration_Protocol_Spec.md` (OPS v1 — S-1…S-9, A1–A6, §7 failure matrix, §8 security, §9 conformance), `docs/phase-2/Mainguard_Environment_Substrate_Contract.md` (ESC / B1 — ESC-I1…ESC-I9, §4 suite), `docs/phase-2/Mainguard_Substrate_WSL2.md` (B2 — WSL stress matrix). Where this document and the master doc disagree, **the master doc wins** and the disagreement is drift to be fixed here — the same precedence rule OPS/ESC apply.
...
| Field | Value |
|---|---|
| Setup / preconditions | Worker `Verified@X`. The user runs a **manual `git merge` in their own client** on the Windows repo, advancing `main` to `X'` out-of-band. The ext4 **bare mirror still shows `X`** (it lags Windows; the daemon cannot read Windows `main` — G-11). Mainguard then merges the worker onto a `main` that is no longer `X`. |
...
| Field | Value |
|---|---|
| Setup / preconditions | The VM (daemon + lease) or `mainguardd` dies **during** the A5 step: after `BeginMerge` and possibly after the Windows-side `git merge` commits, but **before** `ConfirmMerge` records the outcome (dropped response / VM hibernate — WSL2 §2 WT-10/WT-13). |
...
| Field | Value |
|---|---|
| Setup / preconditions | Windows fast-startup / sleep / hibernate stops `MainguardEnv` mid-session; every agent container vanishes; ext4 repos+worktrees survive (WSL2 §2 WT-10/WT-12/WT-13). Attack surface: a worker that was `Verified` when its container died — does its stale state or in-flight work leave a merge enabled or resurrect torn work? |
| Property violated | **S-3** durability + **ESC-I4** (no host-destroying teardown), WSL2 §6 WSL-3. |
| Expected defense | WSL2 §1.2 + WSL-3: on re-boot the `MainguardOsBootstrapper` re-enters idempotently and the `SwarmReconciler` (P2-08, Docker is the sole source of truth) marks vanished agents **`Dead`**, prunes their worktrees, and surfaces disposal — it does **not** silently re-spawn (a re-spawn is a human/Coordinator-gated authority action, S-8; auto-resume risks torn work). Pushed commits on the quarantine bare survive; the merge gate re-reads `CanMerge`, so a `Dead` worker cannot merge. The hibernate stop is **Windows tearing down the VM**, not Mainguard calling a host-wide verb — `TeardownScope` has no host member (ESC-I4 `[STRUCT]`, WSL2 §2 WT-16). |
...
| Field | Value |
|---|---|
| Setup / preconditions | The worker controls its branch, including the **repo-configured test command** the supervisor runs (`VerifyRun{testCommandRef}` — OPS §3.3). It edits `package.json`'s `test` script / a Makefile target / the mainguard test config **inside its own branch** to `exit 0` (or deletes failing tests), producing a green `VerifyResult` that certifies nothing. The `VerificationRecord` is daemon-written and immutable — but it records the exit of a command the worker authored. |
...
*End of Mainguard Orchestration Red-Team Plan (RT v1, draft 2026-07-11).*

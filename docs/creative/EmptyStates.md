...
| **LfsWindow** | git-lfs not on PATH · `not-connected` | `Git LFS isn't installed` | `Install Git LFS to track large files here — GitLoom detects it once it's on your PATH.` | `Button.Secondary` "Recheck" — GitLoom can't install it, and won't pretend to (V-6) | Neutral LFS glyph, **no `DangerBrush`** — a missing tool is a capability gap, not an error (ES-3, V-2, V-6) |
...
| **OperationHistoryWindow** | Journal empty · `empty-yet` | `No operations recorded yet` | `GitLoom journals every commit, merge, and reset here so you can undo them.` | None — the panel fills itself as you work | 48px undo-arc glyph flat; **silent**. Copy states the recovery promise up front (ES-1, V-5) |
